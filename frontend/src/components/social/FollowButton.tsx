...
const FollowButton: React.FC<FollowButtonProps> = ({
  ...
  useEffect(() => {
    setFollowStatus(initialFollowStatus || (initialIsFollowing ? 'active' : 'none'));
  }, [username, initialFollowStatus, initialIsFollowing]);
  ...
  return (
    <button
      ...
      disabled={loading || followStatus === 'rejected'}
    >
      {isRequested ? (
        <>
          <Clock size={18} />
